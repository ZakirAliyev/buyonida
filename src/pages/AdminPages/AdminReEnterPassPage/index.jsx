...
function AdminReEnterPassPage() {
  ...
  const Completionist = () => <span onClick={resendCode} style={{
    color: '#0866FF',
    cursor: 'pointer'
  ...
  const isFormIncomplete = !formik.values.email || !formik.values.newPassword || !formik.values.confirmPassword || otp1.some(digit => digit === '');
  return <section id="adminReEnterPassPage">
            <Helmet>
            ...
            <div className="wrapper">
                ...
                <div className="title">
                    ...
                    <h3>{t("please_enter_your_email_new_password_and_the_otp_code_sent_to_your_email")}</h3>
                </div>
                <form onSubmit={formik.handleSubmit}>
                    <label htmlFor="email">{t("e_mail")}</label>
                    <input id="email" name="email" type="email" onChange={formik.handleChange} value={formik.values.email} readOnly />
                    ...
                    <label htmlFor="confirmPassword">{t("re_enter_new_password")}</label>
                    ...
                    <div className="boxWrapper">
                        {otp1.map((value, index) => <div className="box" key={index}>
                                <input ref={el => inputsRef.current[index] = el} type="text" maxLength="1" value={value} onChange={e => handleChange(index, e.target.value)} onKeyDown={e => handleKeyDown(index, e)} style={{
              textAlign: 'center'
            }} />
                            </div>)}
                    </div>
          ...
          color: '#0866FF'
                        ...
                        <Countdown key={countdownKey} // Countdown'ı sıfırlamak için key
          date={Date.now() + countdownDuration} renderer={renderer} />
                    </div>
                    <button type="submit" disabled={isSubmitting || isFormIncomplete}>
                        {isSubmitting ? <PulseLoader color="white" size={8} loading={isSubmitting} /> : <span>{t("reset_password")}</span>}
                    </button>
                </form>
                <div className="links">
