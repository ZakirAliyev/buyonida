...
import { useGetBasketGetOrCreateQuery, useGetStoreWithSectionsQuery, usePostOrderMutation } from "../../../service/userApi.js";
                    ...
                    <div className="box1 col-6 col-md-12 col-sm-12 col-xs-12">
                        <form className="container2 left-container" onSubmit={handleSubmit}>
                            ...
                            <div className="form-row">
                                ...
                                <div className="form-group">
                                    <input type="text" name="surname" placeholder="Last Name" value={formData.surname} onChange={handleInputChange} />
                                    {errors.surname && <span className="error">{errors.surname}</span>}
                                </div>
                            </div>
                            <div className="form-group">
                                <input type="text" name="address1" placeholder="Address line 1" value={formData.address1} onChange={handleInputChange} />
                                {errors.address1 && <span className="error">{errors.address1}</span>}
                            </div>
                            <div className="form-group">
                                <input type="text" name="address2" placeholder="Address line 2" value={formData.address2} onChange={handleInputChange} />
                            </div>
                            <div className="form-row">
                                <div className="form-group">
                                ...
                                <div className="form-group">
                                    <input type="text" name="postCode" placeholder="Zip Code" value={formData.postCode} onChange={handleInputChange} />
                                    {errors.postCode && <span className="error">{errors.postCode}</span>}
                                </div>
                            </div>
                            <div className="form-group">
                                <textarea name="specialNote" placeholder="Special Note (optional)" value={formData.specialNote} onChange={handleInputChange} rows="4" />
                            </div>
              ...
              return <div key={index} className="cart-item">
                                        <div className="product-block">
                                            <div className="product-image">
                                                <img src={PRODUCT_LOGO + product.imageNames[0]} alt={product.title} onError={e => e.target.src = "/fallback-image.png"} />
                                            </div>
                                            <div className="product-info">
                                                <h4>{product.title}</h4>
                                                {item.basketItemOptions && item.basketItemOptions.map(opt => <p key={opt.productOptionId}>
