...
function MarketProductDetailsPage() {
    ...
    const { data: getAllProductsByMarketId, isLoading: isProductsLoading } =
        useGetAllProductsByMarketIdQuery(marketId, { skip: !marketId });
    const products = getAllProductsByMarketId?.data;

    // Sepeti oluştur ya da getir
    const { refetch: refetchBasket } = useGetBasketGetOrCreateQuery(
        { marketId, uniqueCode },
    ...
    const handlePrevImage = () => {
        setCurrentImageIndex(prev =>
            prev === 0 ? product?.imageNames?.length - 1 : prev - 1
        );
    };

    const handleNextImage = () => {
        setCurrentImageIndex(prev =>
            (prev + 1) % product?.imageNames?.length
        );
    };
    ...
    const handleThumbNext = () => {
        setThumbStartIndex((prev) =>
            prev + visibleCount < totalThumbs ? prev + visibleCount : prev
        );
    };
                            ...
                            <div className="desktop-image-gallery">
                                {totalThumbs > 0 && (
                                    <div className="thumbnail-container">
                                        {thumbStartIndex > 0 && (
                                            <button className="thumb-nav prev" onClick={handleThumbPrev}>
                                                ▲
                                            </button>
                                        )}
                                        {visibleThumbs.map((image, index) => {
                                            ...
                                            return (
                                                <img
                                                    ...
                                                    alt="Thumbnail"
                                                    className={currentImageIndex === globalIndex ? 'active' : ''}
                                                    onClick={() => {
                                        ...
                                        {thumbStartIndex + visibleCount < totalThumbs && (
                                            <button className="thumb-nav next" onClick={handleThumbNext}>
                                                ▼
                                            </button>
                                        )}
                                    </div>
                                ...
                                <div className="main-image-container">
                                    <img
                                        src={PRODUCT_LOGO + product?.imageNames?.[currentImageIndex]}
                                        alt="Main"
                            ...
                            <div className="mobile-image-carousel">
                                <div className="carousel-container">
                                    {product?.imageNames?.map((image, index) => (
                                        <div
                                            key={index}
                                            className={`carousel-slide ${currentImageIndex === index ? 'active' : ''}`}
                                        >
                                ...
                                {totalThumbs > 1 && (
                                    <>
                                        ...
                                        <div className="carousel-indicators">
                                            {product.imageNames.map((_, index) => (
                                                <span
                                                    key={index}
                                        ...
                                        <div className="variantWrapper">
                                            {option?.values &&
                                                option.values.map((opt, idx) => (
                                                    <button
                                                        key={idx}
                                                        className={selectedVariants[option.name] === opt.value ? "optVal active" : "optVal"}
