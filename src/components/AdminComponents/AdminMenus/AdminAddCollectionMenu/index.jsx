...
function AdminAddCollectionMenu() {
  ...
  return <section id="adminAddProductMenu">
            <div className="umumi">
                ...
                <div className="wrapper">
                    <div className="inputWrapper">
                        <label>{t("title")}</label>
                        <input id="productTitle" name="title" placeholder="Enter product title" value={title} onChange={handleTitleChange} className={errors.title ? "errorInput" : ""} />
              ...
              padding: '32px 32px 0 32px'
                ...
                marginTop: '10px'
                  ...
                  margin: '0'
                                    ...
                                    <button onClick={handleBigBoxClick}>{t("axtar")}</button>
                                </div>
                            </div>
                ...
                padding: '16px'
                                    ...
                                    <table className="product-table">
                                        <tbody>
                                        {selectedProducts && selectedProducts.length !== 0 ? selectedProducts.map((item, index) => <tr key={item.id}>
                                                    <td className="checkboxWrapper">{index + 1}{t("")}</td>
                                                    <td className="img">
                                                        <img src={PRODUCT_LOGO + item?.imageNames[0]} alt="Image" />
                                                    </td>
                                                    <td className="productName">{item?.title}</td>
                                                    <td className="btnbtn123">
                                                        {item?.status ? <button>{t("active")}</button> : <button className="deactive">{t("deactive")}</button>}
                                                    </td>
                                                    <td>
                                                        <RxCross2 onClick={() => handleRemoveProduct(item.id)} style={{
                          cursor: 'pointer'
                        }} />
                                                    </td>
                                                </tr>) : <tr style={{
              ...
              padding: '32px 32px 0 32px'
                ...
                justifyContent: 'center'
                  ...
                  cursor: 'pointer'
                                            ...
                                            <img src={coverPreview} alt="Preview" style={{
                    width: '150px',
                    height: '150px',
                    objectFit: 'cover'
                  }} />
                                            <RxCross2 onClick={handleRemoveCoverImage} style={{
                    cursor: 'pointer',
                    position: 'absolute',
                    top: 0,
                    right: 0,
            ...
            {collectionData && <div style={{
      marginTop: '20px',
      backgroundColor: '#f9f9f9',
      padding: '16px'
    }}>
                    <h3>{t("collection_data_preview")}</h3>
                    <pre>{JSON.stringify(collectionData, null, 2)}</pre>
                </div>}

            {/* Modal - məhsul seçimi */}
            <Modal visible={isBigBoxModalOpen} onCancel={() => setIsBigBoxModalOpen(false)} footer={null} width={1000} modalRender={modal => cloneElement(modal, {
      style: {
        ...modal.props.style,
        padding: 0,
        borderRadius: '20px'
                ...
                <AdminCollectionAddProduct selectedProducts={selectedProducts} setSelectedProducts={setSelectedProducts} onDone={handleProductSelectionDone} />
            </Modal>
