...
function AdminCollectionsMenu() {
  ...
  return <section id="adminCollectionsMenu">
            <div className="textWrapper">
                <h2>{t("collections")}</h2>
                <button onClick={() => {
        navigate('/cp/add-collection');
      }}>{t("create_collection")}</button>
            </div>
            <table>
                <thead>
                <tr className="first">
                    <th>
                        <button>{t("all_collections")}</button>
                    </th>
                </tr>
                <tr className="bgbg">
                ...
                <tbody>
                {currentCollections && currentCollections.length !== 0 ? currentCollections.map(item => <tr key={item?.id}>
                            <td className="checkboxWrapper">
                                <input type="checkbox" />
                            </td>
                            <td>
                                <img className="image" src={item?.coverImageUrl ? COLLECTION_LOGO + item.coverImageUrl : "path/to/placeholder/image.jpg"} alt="Image" onError={e => e.target.src = "path/to/placeholder/image.jpg"} />
                            </td>
                            <td onClick={() => {
            ...
            navigate(`/cp/edit-collection/${Cookies.get('chooseMarket')}/${item?.id}`);
                            ...
                            <td>
                                <button className="deleteButton" onClick={() => handleDelete(item?.id)}>{t("delete")}</button>
                            </td>
                        </tr>) : <tr>
                        <td colSpan="6" style={{
            textAlign: 'center',
            padding: '20px'
          }}>{t("there_are_no_collections_available")}</td>
                    </tr>}
                </tbody>
            </table>
            {totalCollections > collectionsPerPage && <div className="pagination">
