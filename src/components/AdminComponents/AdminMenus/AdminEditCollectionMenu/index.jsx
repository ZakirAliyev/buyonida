...
import { useEditCollectionMutation, useGetAllProductsByMarketIdQuery, useGetCollectionByMarketIdQuery } from "../../../../service/userApi.js";
...
function AdminEditCollectionMenu() {
  ...
  return <section id="adminEditCollectionMenu">
            <div className="umumi">
                <div className="abso">
                    <span>{t("category_status")}</span>
                    <button className={isActive ? "active" : "inactive"} onClick={handleStatusToggle}>
                ...
                <div className="lineWrapper">
                    ...
                    <h1>{t("edit_collection")}</h1>
                </div>

                <div className="wrapper">
                    <div className="inputWrapper">
                        <label>{t("title")}</label>
                        <input id="productTitle" name="title" placeholder="Enter collection title" value={title} onChange={handleTitleChange} className={errors.title ? "errorInput" : ""} />
                    ...
                    <div className="inputWrapper">
                        <label htmlFor="productTitle">{t("description")}</label>
            ...
            padding: 0
              ...
              padding: '32px 32px 0 32px'
                                ...
                                <div className="inputWrapper1">
                                    ...
                                    <button onClick={handleBigBoxClick} className={"menimButton"}>{t("axtar")}</button>
                                </div>
                            </div>
                            ...
                            <div className="textWrapper">
                                <div className="inputWrapper1">
                                    <table className="product-table">
                                        <tbody>
                                        {filteredProducts?.length > 0 ? filteredProducts?.map((item, index) => <tr key={item.id}>
                                                    <td className="checkboxWrapper">{index + 1}{t("")}</td>
                                                    <td className="img">
                                                        <img src={item.imageNames && item.imageNames.length > 0 ? `${PRODUCT_LOGO}${item.imageNames[0]}` : ""} alt={item.title} onError={e => e.target.src = PRODUCT_LOGO + item?.imageNames[0]} />
                                                    </td>
              ...
              padding: '32px 32px 0 32px'
                ...
                justifyContent: 'center'
                  ...
                  alignItems: 'center'
                                            ...
                                            <img src={coverFile ? coverPreview : `${COLLECTION_LOGO}${coverPreview}`} alt="Cover" style={{
                    width: '150px',
                    height: '150px',
                    borderRadius: '15px',
                    objectFit: 'cover'
                    ...
                    cursor: 'pointer'
                  }} onClick={handleRemoveCoverImage}>{t("remove_image")}</button>
                                        </div> : <Dragger {...draggerProps} style={{
            ...
            <div className="buttin1">
                <button onClick={handleSaveChanges}>{loading ? <PulseLoader color={"white"} size={8} /> : <>{t("save_changes")}</>}</button>
            </div>

            <Modal visible={isBigBoxModalOpen} onCancel={handleBigBoxClick} footer={null} width={1000} modalRender={modal => cloneElement(modal, {
      style: {
        ...modal.props.style,
        padding: 0,
        borderRadius: '20px'
                ...
                <AdminCategoryAddProduct selectedProducts={selectedProductIds} onProductSelect={handleProductSelect} allProducts={myProducts || []} // Pass all products instead of collection.products
      />
            </Modal>

            <CategoryModal isOpen={false} onClose={() => {}} onSave={() => {}} />
            <ToastContainer />
        </section>;
}
