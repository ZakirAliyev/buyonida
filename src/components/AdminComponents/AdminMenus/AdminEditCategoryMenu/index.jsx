...
function AdminEditCategoryMenu() {
  ...
  const getImageSrc = image => {
    if (!image) return "";
    return typeof image === 'string' ? CATEGORY_LOGO + image : URL.createObjectURL(image);
  };
  ...
  const handleImageUpload = ({
    ...
    if (file) {
      ...
      if (errors.imageFiles) setErrors({
        ...errors,
        imageFiles: ""
      });
  ...
  return <section id="adminEditCategoryMenu">
            <div className="umumi">
                {/* Category status bar */}
                <div className="abso">
                    <span>{t("category_status")}</span>
                    <button className={isActive ? 'but123ton' : 'dtnButton'} onClick={handleStatusToggle}>
                        {isActive ? "Active" : "Inactive"}
                    </button>
                    {errors.isActive && <span className="error-text">
                ...
                <div className="row">
                    ...
                    <div className="pd0 col-6" style={{
          paddingRight: "8px"
        }}>
                        <div className="wrapper">
                            <div className="inputWrapper">
                                <label>{t("title")}</label>
                                <input id="productTitle" name="title" placeholder="Enter category title" value={title} onChange={handleTitleChange} className={errors.title ? "errorInput" : ""} />
            ...
            padding: 0
              ...
              padding: "32px 32px 0 32px"
                ...
                width: "100%"
                                    ...
                                    {/* Image preview grid */}
                                    {imageFiles.length > 0 && <div style={{
                  display: 'flex',
                  flexWrap: 'wrap',
                  gap: '16px',
                  margin: '16px'
                }}>
                                            {imageFiles.map((image, index) => <div key={index} className="imageWrapper" style={{
                    position: "relative"
                  }}>
                                                    <img src={getImageSrc(image)} alt={`Category-${index}`} style={{
                      width: '100%',
                      borderRadius: "10px"
                    }} />
                                                    <span className="removeImage" onClick={() => handleRemoveImage(index)} style={{
                      position: "absolute",
                      top: "-8px",
                      right: "-8px",
                      cursor: "pointer",
                      ...
                      padding: "2px"
                                    ...
                                    {/* Image uploader */}
                                    {imageFiles.length < 1 && <Dragger name="file" multiple={true} beforeUpload={() => false} onChange={handleImageUpload} showUploadList={false} style={{
                  ...
                  borderWidth: "2px"
                                            ...
                                            <p className="ant-upload-drag-icon">
                                                <button type="button">{t("add_media")}</button>
                                            </p>
                                            <p className="ant-upload-text">{t("click_or_drag_files_to_this_area_to_upload")}</p>
                                        </Dragger>}
                                    {errors.imageFiles && <span className="error-text">
                                            <AiOutlineExclamationCircle className="icon" />
                                            {errors.imageFiles}
                                        </span>}
                                </div>
                ...
                {errors.id && <span className="error-text">
                        <AiOutlineExclamationCircle className="icon" />
                        {errors.id}
                    </span>}
                {errors.marketId && <span className="error-text">
                        <AiOutlineExclamationCircle className="icon" />
                        {errors.marketId}
                    </span>}
            </div>
        ...
        color: "white"
                    ...
                    {editLoading ? <PulseLoader color={"white"} /> : <>{t("save_changes")}</>}
                </button>
            </div>
