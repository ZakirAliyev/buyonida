...
import {FaCodeBranch, FaStoreAlt} from 'react-icons/fa';
...
function AdminLeftMenu() {
    ...
    const settingsPaths = ['/cp/settings', '/cp/general', '/cp/balance-payout', '/cp/payments', '/cp/shipping-and-delivery', '/cp/policies', '/cp/analytics-integration'];
    ...
    return <section id={'adminLeftMenu'}>
        <div className={'wrapper1'}>
            {isSettingsPage ? <>
                ...
                <Link to={'/cp/analytics-integration'} className={`link ${isSelected('/cp/analytics-integration')}`}>
                    <FaCodeBranch className={'icon'}/>{t("analytics_integration")}</Link>
            </> : <>
                <Link to={'/cp/home'} className={`link ${isSelected('/cp/home')}`}>
                    <IoMdHome className={'icon'}/>{t("home")}</Link>
                <div className="menu-item">
                    <div className={`link ${isSelected(['/cp/orders', '/cp/abandoned-checkouts'])}`}
                    ...
                    <div className={`wrrara ${openSubmenu === 'orders' ? 'open' : ''}`}>
                        ...
                        <div className="submenu">
                            <Link to={'/cp/orders'}
                                  className={`sublink ${location.pathname === '/cp/orders' ? 'selected' : ''}`}>{t("orders")}</Link>
                            <Link to={'/cp/abandoned-checkouts'}
                                  className={`sublink ${location.pathname === '/cp/abandoned-checkouts' ? 'selected' : ''}`}>{t("abandoned_checkouts")}</Link>
                        </div>
                    </div>
                </div>
                <div className="menu-item">
                    <div className={`link ${isSelected(['/cp/products', '/cp/categories', '/cp/collections'])}`}
                    ...
                    <div className={`wrrara ${openSubmenu === 'products' ? 'open' : ''}`}>
                        ...
                        <div className="submenu">
                            <Link to={'/cp/products'}
                                  className={`sublink ${location.pathname === '/cp/products' ? 'selected' : ''}`}>{t("products")}</Link>
                            <Link to={'/cp/categories'}
                                  className={`sublink ${location.pathname === '/cp/categories' ? 'selected' : ''}`}>{t("categories")}</Link>
                            <Link to={'/cp/collections'}
                                  className={`sublink ${location.pathname === '/cp/collections' ? 'selected' : ''}`}>{t("collections")}</Link>
                        </div>
                    </div>
                </div>
                <Link to={`/cp/analytics/${Cookies.get('chooseMarket')}`}
                      className={`link ${isSelected('/cp/analytics')}`}>
        ...
        {!isSettingsPage ? <Link to={'/cp/settings'} className={`link1 link ${isSelected('/cp/settings')}`}>
                <IoMdSettings className={'icon'}/>{t("settings")}</Link> :
