...
function AdminLoginForm() {
  ...
  return <section id={"adminLoginForm"}>
          ...
          position: 'relative'
                        ...
                        <input type={showPassword ? 'text' : 'password'} name="password" onChange={formik.handleChange} value={formik.values.password} style={{
            width: '100%'
          }} />
                        <div onClick={() => setShowPassword(prev => !prev)} style={{
            position: 'absolute',
            right: '15px',
            top: '15px',
            cursor: 'pointer'
              ...
              fontSize: '20px'
                    ...
                    <Link to={`/forgot-pass`} className={"forgotPass"}>{t("forgot_password")}</Link>

                    <button type="submit" onClick={handleValidationClick}>
                        {!isSubmitting ? <span>{t("log_in")}</span> : <PulseLoader color={'white'} size={8} loading={isSubmitting} />}
                    </button>
                </form>
        ...
        cursor: 'pointer'
          ...
          theme: 'dark'
                ...
                <nav>{t("new_to_buyonida")}<Link to={`/register`} className={"link"}>{t("get_started")}</Link>
