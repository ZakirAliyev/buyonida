...
function MarketCard({number, product,palet,marketName}) {
    ...
    return (
        <div className={`col-${newNumber}-60 `} style={{padding:"8px"}} onClick={() => {
            navigate(`/@${marketName}/product/${product?.id}`)
        }}>
            <section id={"marketCard3"} style={{backgroundColor:palet ? (`${palet[0]?.cardBgColor}`) : ("#ffffff"),color:palet ? (`${palet[0]?.cardTextColor}`) : ("#000000")}}>
                   <div className={"img"}>
