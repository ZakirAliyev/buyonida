...
export default function MarketSwiperHero({
  ...
  const sortedBannerItems = data ? [...data].sort((a, b) => a.displayOrderId - b.displayOrderId) : [];
      ...
      height: navHeight
            ...
            <div id="marketSwiperHero">
                <Swiper className="mySwiper">
                    {sortedBannerItems.map(item => <SwiperSlide key={item.id}>
                            <div className="bgImage" style={{
            background: item?.imageName ? `linear-gradient(0deg, rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), url(${BANNER_LOGO + item.imageName})` : "none",
            ...
            backgroundColor: item?.imageName ? "transparent" : "#d4a017"
                                ...
                                <div className="container">
                                    <div className="bannerText">
                                        <h2>{item?.title}</h2>
                                        <p>{item?.subtitle}</p>
                                        <button onClick={() => window.location.assign(item?.buttonLink)}>{t("shop_now")}</button>
                                    </div>
                                </div>
                            </div>
                        </SwiperSlide>)}
