...
import { useGetAllCategoriesByMarketIdQuery, useGetAllCollectionsByMarketIdQuery, useGetStoreWithSectionsQuery } from "../../../service/userApi.js";
...
const CustomModal = ({
  ...
  const filteredOptions = sectionType ? options.filter(opt => opt.type === sectionType) : [];
  return <div className="custom-modal-overlay">
          ...
          visibility: 'hidden'
                    ...
                    <h3>{t("add_new_section")}</h3>
                    <button className="custom-modal-close" onClick={onClose}>{t("")}</button>
                </div>
                <div className="custom-modal-body">
                    <div className="inputWrapper">
                        <div className="label">{t("section_type")}</div>
                        <Select value={sectionType} style={{
            width: '100%'
          }} onChange={value => {
            ...
            value: 'category',
            label: 'Category'
          }]} placeholder="Select a type" allowClear />
                    </div>
                    <div className="inputWrapper">
                        <div className="label">{t("select_item")}</div>
                        <Select value={sectionValue} style={{
            width: '100%'
          }} onChange={value => setSectionValue(value)} options={filteredOptions} placeholder="Select an item" disabled={!sectionType} // Disable until sectionType is selected
          />
                    </div>
                </div>
                <div className="custom-modal-footer">
                    <button className="custom-modal-button cancel" onClick={onClose}>{t("cancel")}</button>
                    <button className="custom-modal-button confirm" onClick={onConfirm} disabled={!sectionValue}>{t("add_section")}</button>
                </div>
            </div>
        </div>;
};
function SectionsManager({
  ...
  useEffect(() => {
    if (storeData?.data?.sections) {
      const filtered = storeData.data.sections.filter(s => s.$type && (s.$type.toLowerCase() === "category" || s.$type.toLowerCase() === "collection"));
  ...
  return <div id="sectionsManager">
            <div className="sections-manager-container">
                <DragDropContext onDragEnd={onDragEnd}>
                    <Droppable droppableId="droppable-sections">
                        {provided => <div className="sections-list" ref={provided.innerRef} {...provided.droppableProps}>
                                {sections.map((section, index) => <Draggable key={section.localId} draggableId={section.localId} index={index}>
                                        {providedDrag => <div className="section-item" ref={providedDrag.innerRef} {...providedDrag.draggableProps} {...providedDrag.dragHandleProps}>
                                                <div style={{
                  display: "flex",
                  alignItems: "center",
                  padding: "0 8px",
                  ...
                  borderRadius: "10px"
                                                    ...
                                                    <div className="section-header">
                                                        <span className="section-title">
                                                            {section.type === "collection" ? "Collection list" : "Category list"}
                                                        </span>
                                                        <button className="remove-btn" onClick={() => handleRemoveSection(index)}>{t("")}</button>
                                                    </div>
                                                    <div className="section-controls">
                                                        <div className="inputWrapper">
                                                            <div className="label">{t("selected_item")}</div>
                                                            <Select value={section.value} style={{
                        width: 150
                      }} onChange={value => handleSectionChange(index, value)} options={combinedOptions} />
                                                        </div>
                                                        <div className="inputWrapper1123 cardsInRowControl">
                                                            <div className="label cardsInRowLabel">{t("show_cards_in_a_row")}</div>
                                                            <div className="rangeContainer">
                                                                <span>{t("3")}</span>
                                                                <input type="range" min="3" max="6" step="1" value={section.cardsInRow} onChange={e => handleCardsInRowChange(index, e.target.value)} />
                                                                <span>{t("6")}</span>
                                                            </div>
                                                        </div>
                                                    </div>
        ...
        padding: "16px"
                    ...
                    <div className="h3">{t("add_a_section")}</div>
                    <HiOutlinePlusSm className="csplus1" onClick={() => setIsModalVisible(true)} />
                </div>
            </div>
            <CustomModal isVisible={isModalVisible} onClose={() => {
      setIsModalVisible(false);
      setNewSectionValue(null);
      setNewSectionType(null); // Reset section type on close
    }} onConfirm={handleCreateNewSection} sectionType={newSectionType} setSectionType={setNewSectionType} sectionValue={newSectionValue} setSectionValue={setNewSectionValue} options={combinedOptions} />
        </div>;
}
