...
import { useGetStoreWithSectionsQuery, usePostSectionBannerItemMutation, usePutSectionsMutation, useEditSectionBannerItemMutation, useDeleteSectionBannerItemMutation } from "../../../service/userApi.js";
...
function CustomizeStoreScreens() {
  ...
  useEffect(() => {
    if (sectionsData && tabs.length === 0) {
      const bannerSection = sectionsData.sections?.find(section => section.sectionType === "Banner");
      if (bannerSection) {
  ...
  function getFinalJson() {
    const bannerSectionFromAPI = sectionsData?.sections?.find(section => section.sectionType === "Banner");
    ...
    const categorySections = updatedSections.filter(section => section.type === "category").map(section => ({
      id: section.id || null,
      marketId: sectionsData ? sectionsData.id : null,
    ...
    const collectionSections = updatedSections.filter(section => section.type === "collection").map(section => ({
      id: section.id || null,
      marketId: sectionsData ? sectionsData.id : null,
      displayOrderId: section.displayOrderId,
  ...
  async function handlePutSection(finalJson) {
    try {
      ...
      for (let index = 0; index < tabData.length; index++) {
        ...
        const isFullyFilled = tab.title?.trim() && tab.subTitle?.trim() && tab.buttonLink?.trim() && tab.image?.length > 0;
        if (!isFullyFilled) {
  ...
  async function handleDeleteBannerItem(tabId, label) {
    try {
      ...
      if (response?.statusCode === 200) {
        ...
        setTabs(updatedTabs);
        setTabData(prev => prev.filter(item => item.id !== tabId).map((item, idx) => ({
          ...item,
          displayOrderId: idx + 1
        })));
  ...
  const updateTabData = (id, field, value) => {
    setTabData(prev => prev.map(item => item.id === id ? {
      ...item,
      [field]: value
    } : item));
  };
  const onImageChange = tabId => ({
    fileList: newFileList
  }) => {
    setTabData(prev => prev.map(item => item.id === tabId ? {
      ...item,
  ...
  const removeTab = index => {
    ...
    setTabs(updatedTabs);
    setTabData(prev => {
      const newData = prev.filter(item => item.id !== tabToRemove.id).map((item, idx) => ({
        ...item,
        displayOrderId: idx + 1
      }));
      return newData;
  ...
  return <section id="customizeStoreScreens">
            <div className="row">
                <div className="firstScreen col-14-60 my-scrollable-element">
                    <div className="choose row main-tabs">
                        <div className={`sections pd-8 col-6 clickable ${activeMainTab === "sections" ? "selected" : ""}`} onClick={() => setActiveMainTab("sections")}>{t("sections")}</div>
                        <div className={`settings pd-8 col-6 clickable ${activeMainTab === "settings" ? "selected" : ""}`} onClick={() => setActiveMainTab("settings")}>{t("settings")}</div>
                    </div>

                    <div style={{
          display: activeMainTab === "sections" ? "block" : "none"
        }}>
                            ...
                            <Tabs selectedIndex={activeBannerIndex} onSelect={index => setActiveBannerIndex(index)}>
                                <TabList>
                                    {tabs.map((tab, index) => <Tab key={index}>
                                            {index > 0 && <FiArrowLeft className="csicon arrow-icon" onClick={e => {
                    e.stopPropagation();
                    moveTabLeft(index);
                  }} />}
                                            ...
                                            {index < tabs.length - 1 && <FiArrowRight className="csicon arrow-icon" onClick={e => {
                    e.stopPropagation();
                    moveTabRight(index);
                  }} />}
                                            {menuOpenTabIndex === index && <div className="tab-menu">
                                                    <button onClick={e => {
                      e.stopPropagation();
                      removeTab(index);
                      ...
                      setMenuOpenTabIndex(null);
                                ...
                                {tabs.map((tab, index) => <TabPanel key={index}>
                                        <div className="inputWrapper">
                                            ...
                                            <input value={getTabData(tab.id)?.title || ""} onChange={e => updateTabData(tab.id, "title", e.target.value)} required />
                                        </div>
                                        <div className="inputWrapper">
                                            <div className="label">{t("sub_title")}</div>
                                            <input value={getTabData(tab.id)?.subTitle || ""} onChange={e => updateTabData(tab.id, "subTitle", e.target.value)} required />
                                        </div>
                                        <div className="inputWrapper">
                                            <div className="label">{t("button_link")}</div>
                                            <input value={getTabData(tab.id)?.buttonLink || ""} onChange={e => updateTabData(tab.id, "buttonLink", e.target.value)} placeholder="Enter link URL" required />
                                        </div>
                  ...
                  alignItems: "center"
                  ...
                  }} listType="picture-card" fileList={getTabData(tab.id)?.image || []} onChange={onImageChange(tab.id)} onPreview={onPreview} maxCount={1} required>
                                                {(getTabData(tab.id)?.image || []).length < 1 && "+ Upload"}
                                            </Upload>
                                        </div>
                                        {tabs.length > 1 && !tab.isNew && <div className="inputWrapper">
                                                <button className="delete-banner-btn" style={{
                    background: "#ff4d4f",
                    color: "white",
                    padding: "8px 16px",
                    border: "none",
                    ...
                    cursor: "pointer",
                    marginTop: '20px'
                  }} onClick={() => handleDeleteBannerItem(tab.id, fixedLabels[index] || `Tab ${index + 1}`)}>{t("delete_banner_item")}</button>
                                            </div>}
                                    </TabPanel>)}
                            </Tabs>
                        </div>
            ...
            padding: '0 16px'
                                ...
                                <button className="save-sections-btn-123" onClick={() => handlePutSection(getFinalJson())}>{t("save_changes")}</button>
                            </div>
                    </div>
                    <CustomizeStoreSettingTab activeMainTab={activeMainTab} setCustomLogo={setCustomLogo} setCustomLogoWidth={setCustomLogoWidth} />
                </div>
                <div className="lastScreen col-46-60">
                    <div className="pd-16">
                        <CustomizeStoreMarketHomePage swipers={tabData} sections={finalSections} customLogo={customLogo} customLogoWidth={customLogoWidth} />
                    </div>
                </div>
            </div>
        </section>;
