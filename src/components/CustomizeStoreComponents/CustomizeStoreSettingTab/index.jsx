...
import { useGetPaletteByMarketIdQuery, useGetStoreByIdQuery, useGetStoreWithSectionsQuery, usePostPaletteMutation, usePostSettingBrandingMutation, useUpdatePaletteMutation, useDeletePaletteMutation, useSelectPaletteMutation, useSelectFontNameMutation } from "../../../service/userApi.js";
...
function AccordionItem({
  ...
  return <div className="accordion-item" style={{
...
function CustomizeStoreSettingTab({
  activeMainTab,
  setCustomLogo,
  setCustomLogoWidth
}) {
  ...
  const {
    data: getPaletteByMarketId,
    refetch: refetchPalettes
  } = useGetPaletteByMarketIdQuery(chosenMarket);
  ...
  const {
    ...
    refetch: refetchStoreWithSections
  ...
  const handleSelectPalette = async id => {
    ...
    try {
      ...
      const response = await selectPalette({
        marketId,
        paletId: id
      }).unwrap();
  ...
  const handleSaveFont = async () => {
    try {
      ...
      const response = await postSettingFontName({
        marketId,
        fontName
      }).unwrap();
      if (response?.statusCode === 200) {
  ...
  return <section id="customizeStoreSettingTab">
            <div style={{
      display: activeMainTab === "settings" ? "block" : "none"
    }}>
                <div className="settingsContent">
                    <AccordionItem title="Brending" isOpen={openAccordions["Brending"]} onToggle={() => setOpenAccordions(prev => ({
          ...prev,
          Brending: !prev["Brending"]
        }))}>
                        <div className="myWrapper">
                            <div className="name">{t("logo")}</div>
              ...
              setTimeout(() => {
                onSuccess("ok");
            ...
            }} listType="picture-card" fileList={logoFileList} onChange={handleLogoChange} onPreview={onPreviewFile} maxCount={1}>
                        ...
                        <div className="myWrapper">
                            <div className="name">{t("width")}<span>{localLogoWidth}{t("px")}</span>
                            </div>
                            <input type="range" min="50" max="300" value={localLogoWidth} onChange={handleWidthChange} />
                        </div>
                        <div className="myWrapper">
                            <div className="name">{t("favicon")}</div>
                            <Upload customRequest={({
              onSuccess
            }) => {
              setTimeout(() => {
            ...
            }} listType="picture-card" fileList={faviconFileList} onChange={({
              fileList
            }) => setFaviconFileList(fileList)} onPreview={onPreviewFile} maxCount={1}>
                        ...
                        <div className="myWrapper">
                            <div className="name">{t("about_store")}</div>
                            <textarea type="text" rows={6} value={aboutStore} onChange={e => setAboutStore(e.target.value)} />
                        </div>
                        <div className="myWrapper">
                            <Button onClick={handleSaveBranding}>{t("save_branding")}</Button>
                        </div>
                    </AccordionItem>

                    <AccordionItem title="Color palette" isOpen={openAccordions["Color palette"]} onToggle={() => setOpenAccordions(prev => ({
          ...prev,
          "Color palette": !prev["Color palette"]
        }))}>
                        {editingSchemeId || showAddColorForm ? <div className="row">
                                <div className="col-12">
                                    <div className="edit-form">
                                        <h3>
                                            {editingSchemeId ? `Edit Palette - ${editingPalette?.name}` : "Add New Palette"}
                                        </h3>
                                        <div className="form-group">
                                            <label>{t("palette_name")}</label>
                    ...
                    name: e.target.value
                                        ...
                                        <Form ref={formRef} initialValues={formData}>
                                            <Form.Item name="textColor" label="Text Color">
                                                <CustomColorPicker onChange={color => setFormData(prev => ({
                      ...prev,
                      textColor: color
                    }))} />
                                            </Form.Item>
                      ...
                      buttonTextColor: color
                                        ...
                                        <div className="edit-buttons">
                                            <Button onClick={editingSchemeId ? handleUpdate : handleCreate}>{t("save")}</Button>
                                            {editingSchemeId && <Button onClick={() => handleSelectPalette(editingSchemeId)}>{t("select")}</Button>}
                                            {editingSchemeId && palette.length > 1 && <Button onClick={() => handleDeletePalette(editingSchemeId)}>{t("delete")}</Button>}
                                ...
                                <div className="row">
                                    {palette && palette.length > 0 && palette.map(item => <div key={item.id} className="col-4">
                                                <div className="wrapper2" onClick={() => setEditingSchemeId(item.id)}>
                                                    <div className="box" style={{
                    backgroundColor: item.backgroundColor,
                    color: item.textColor
                  }}>
                                                        {selectedPaletteId === item.id && <FaCheck className="selected-icon" />}{t("aa")}<div style={{
                      display: "flex",
                      gap: "5px"
                    }}>
                                                            <div className="rengBoxu" style={{
                        backgroundColor: item.navbarBgColor
                      }}></div>
                                                            <div className="rengBoxu" style={{
                        backgroundColor: item.navbarTextColor
                      }}></div>
                                                        </div>
                                                    </div>
                                    ...
                                    {!showAddColorForm && editingSchemeId === null && <div className="col-4">
                                            <div className="wrapper2" onClick={toggleAddColorForm} style={{
                  cursor: "pointer"
                }}>
                                                <div className="box box1 plus">{t("")}</div>
                                                <div className="myName">{t("add_color")}</div>
                                            </div>
                                        </div>}
                                </div>
                            </>}
                    ...
                    <AccordionItem title="Font" isOpen={openAccordions["Font"]} onToggle={() => setOpenAccordions(prev => ({
          ...prev,
          Font: !prev["Font"]
        }))}>
                        <div className="myWrapper1">
                            <div className="name">{t("choose_font_family")}</div>
                            <select value={selectedFont} onChange={e => {
              setSelectedFont(e.target.value);
            }}>
                                <option value="Poppins">{t("poppins")}</option>
                                <option value="Space Grotesk">{t("space_grotesk")}</option>
                            </select>
                        </div>
                        <div className="myWrapper1">
                            <Button onClick={handleSaveFont}>{t("save_font")}</Button>
                        </div>
                    </AccordionItem>
                </div>
            </div>
