...
function BlogDetails() {
    ...
    const id = params?.id;
    const { data: getBlogById, isLoading: isBlogLoading, isFetching: isBlogFetching } = useGetBlogByIdQuery(id);
    const blog = getBlogById?.data;

    const { data: getAllBlogs, isLoading: isAllBlogsLoading, isFetching: isAllBlogsFetching } = useGetAllBlogsQuery();
    ...
    if (showLoading) {
        return (
            ...
            >
                <img style={{ width: '250px' }} src={image2} alt="Logo" />
                <PulseLoader />
            </div>
        );
    }
            ...
            <div className={'container6'}>
                ...
                >
                    ...
                    <div className={'author-info'}>
                        ...
                        <div>
                            <p className={'author-name'}>Zakir Aliyev</p>
                            <p className={'author-details'}>
                                Published in Buyonida.com • {blog?.date || 'Unknown Date'} • {readTime} min read
                            </p>
                        </div>
                    </div>
                    <img
                        src={BLOG_CARD_IMAGE + (blog?.bannerImageName || blog?.cardImageName)}
                        alt={blog?.titleEN}
                    ...
                    <div className={'row'}>
                        {relatedBlogs.length > 0 ? (
                            relatedBlogs.map((relatedBlog) => (
                                <BlogCard
                                    colCount={6}
                                    key={relatedBlog.id} // Use blog ID as key
                                    blog={relatedBlog}
                                />
                            ))
                        ) : (
